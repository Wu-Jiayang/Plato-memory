import paddle
from paddlenlp.transformers import UnifiedTransformerTokenizer, LinearDecayWithWarmup
...
load_from = 'plato_memory_model'
save_to = 'plato_memory_model'
...
train_dataset = MemoryDataset(tokenizer, train_dataset, batch_size=batch_size)
train_dataloader = paddle.io.DataLoader(train_dataset, return_list=True, batch_size=None, num_workers=num_workers)
eval_dataset = MemoryDataset(tokenizer, eval_dataset, batch_size=batch_size)
...
opt = paddle.optimizer.AdamW(lr_scheduler, parameters=model.parameters() + [init_state], apply_decay_param_fun=lambda x: x in decay_params)


def compute(data):
    correct, correct_distance, loss = 0, 0, 0
    inputs = data['encoded_inputs']
    state_update_inputs = data['state_update_inputs']
    distances = data['distances']
    ask_idxs = data['ask_idxs']
    total = distances.shape[0]      # batch_size
    state = paddle.expand(init_state, (total, state_size, 768))
    ...
    for i, model_input in enumerate(inputs):
        state_update_input = state_update_inputs[i]
        model_input['state'] = state
        state_update_input['state'] = state
        output, _ = model(**model_input)
        _, state = model(**state_update_input)
        mask = paddle.cast(model_input['token_type_ids'], dtype='bool')
        ...
        if paddle.any(ask_idxs == i):   # 根据answer评估记忆力
            ...
            loss += loss_fn(answer_pred[answer_mask], answer_true[answer_mask])     # 着重训练记忆力
        ...
        y_true = y_true[mask]       # 只计算token_type_ids为1的loss
